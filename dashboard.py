...
if (instrument == 'coin') or (instrument == 'events') or (instrument == 'exchanges') or (instrument == 'today') or (instrument == 'ticker') or (instrument == 'twitter') or (instrument == 'markets') or (instrument == 'price_converter'):
    id = st.sidebar.text_input('Coin ID', 'btc-bitcoin')
    try:
        data = getattr(client, instrument)(id)
    except:
        if instrument == 'price_converter':
            quote_curr = st.sidebar.text_input('Quote Currency ID', 'usd-us-dollars')
